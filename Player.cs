...
namespace APEng3Final
{
    class Player
    {
        ...
        public Player()
        {
            ...
            velocity.X = 0.000004f;
        }
        Texture block;
        public void Load()
        {
            playerSprite = LoadTexture("Sprites/Player_Idle.png");
        ...
        public void Draw()
        {
            ...
            DrawText($"Points {points}", 10, 70, 20, WHITE);
            DrawText($"Highscore {highscore}", 10, 90, 20, WHITE);

            bool drawBlock = IsKeyDown(KeyboardKey.KEY_LEFT_CONTROL) || IsKeyDown(KeyboardKey.KEY_RIGHT_CONTROL);
        ...
        public bool ItemCollision(Rectangle colHitBox)
        {
            Rectangle hitbox = new Rectangle((int)position.X, (int)position.Y, 32, 32);
            return CheckCollisionRecs(hitbox, colHitBox);
        }
    }
}